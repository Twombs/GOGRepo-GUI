...
; EnableDisableControls($state), FillTheGamesList(), GetWindowPosition()
; ParseTheManifest()
...
;
Global $a, $ans, $array, $auto, $bigpic, $blackjpg, $c, $check, $chunk, $chunks, $cookies, $cover, $date, $downlist
Global $extras, $file, $files, $game, $gamefold, $games, $gamesfle, $gogrepo, $GOGRepoGUI, $height, $icoD, $icoF, $icoI
Global $icoT, $icoX, $image, $imgfle, $infofle, $inifle, $lang, $left, $line, $lines, $logfle, $manifest, $name, $open
Global $OS, $OSget, $pid, $read, $res, $segment, $shell, $shutdown, $split, $state, $style, $text, $textdump, $title
Global $titlist, $top, $tot, $user, $val, $validate, $version, $width, $winpos, $xpos, $ypos
...
Func MainGUI()
	...
	;
	Local $add, $all, $alpha, $dll, $every, $exist, $gamepic, $gamesfold, $let, $mpos, $OSes, $pth, $show, $verify
	;
	$width = 590
	$height = 405
								...
								Else
									...
									If $exist <> "" Then
										$ans = MsgBox(262177, "Replace Query", _
											...
											"NOTE - This does not change its downloading order.", 0, $GOGRepoGUI)
										If $ans = 1 Then
											$add = 1
											$tot = $exist
										EndIf
...
Func QueueGUI()
	...
	Local $Button_remall, $Button_remove, $Button_start, $Button_stop, $Checkbox_check, $Checkbox_dos, $Checkbox_files
	Local $Checkbox_image, $Checkbox_other, $Checkbox_start, $Checkbox_stop, $Combo_shutdown, $Group_auto, $Group_done
	Local $Group_download, $Group_info, $Group_progress, $Group_shutdown, $Group_stop, $Group_waiting, $Input_destination
	Local $Input_download, $Input_lang, $Input_OS, $List_done, $List_waiting, $Progress_bar
	;
	Local $ind, $QueueGUI, $section, $swap, $t, $titles
	;
	...
	;
	$Button_remall = GuiCtrlCreateButton("Remove ALL", 450, 140, 80, 22)
	GUICtrlSetFont($Button_remall, 7, 600, 0, "Small Fonts")
	GUICtrlSetTip($Button_remall, "Remove all downloads from the list!")
	;
	...
	;
	$Button_edit = GuiCtrlCreateButton("UPDATE" & @LF & "Selected", 450, 210, 80, 39, $BS_MULTILINE)
		...
		If Not @error Then
			If $titles[0] > 1 Then
				...
				For $t = 2 To $titles[0]
					...
					EndIf
				Next
				If $tot > 0 Then GUICtrlSetData($Group_waiting, "Games To Download  (" & $tot & ")")
			EndIf
		EndIf
	Else
		...
		GUICtrlSetState($Button_remall, $GUI_DISABLE)
			...
			If $title <> "" Then
				...
				If $title = $val Then
					...
					If Not @error Then
						...
						EndIf
					EndIf
					$tot = _GUICtrlListBox_DeleteString($List_waiting, $ind)
